    ...
    <div className="min-h-screen bg-background" style={{ minHeight: '100dvh' }}>
      ...
      <header className="w-full border-b border-border/60 bg-card/60 backdrop-blur supports-[backdrop-filter]:bg-card/40 sticky top-0 z-40">
        <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 h-14 md:h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            <Button
              variant="ghost"
              className="hidden sm:inline-flex h-8 md:h-10 text-sm"
              onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
            >
              Sign in
            </Button>
            <Button
              className="bg-primary text-primary-foreground hover:bg-primary/90 h-8 md:h-10 px-3 md:px-4 text-xs md:text-sm"
              onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
            >
              Get started
            </Button>
          ...
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
            ...
            <div className="space-y-6 md:space-y-8 text-center lg:text-left">
              <div className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-card/60 px-3 py-1 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1 md:gap-2"><Wand2 className="h-3 w-3 md:h-3.5 md:w-3.5" /> AI‑Powered Creative Suite</span>
              </div>
              ...
              {/* Feature chips */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3 max-w-2xl mx-auto lg:mx-0">
                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-lg border border-border bg-card/60">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-blue-600/15 flex items-center justify-center flex-shrink-0"><Wand2 className="h-4 w-4 md:h-5 md:w-5 text-blue-400" /></div>
                  <div>
                    <p className="text-xs md:text-sm font-medium text-foreground">Smart Image Editing</p>
                    <p className="text-xs text-muted-foreground">AI‑powered photo manipulation</p>
                  </div>
                </div>

                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-lg border border-border bg-card/60">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-purple-600/15 flex items-center justify-center flex-shrink-0"><Video className="h-4 w-4 md:h-5 md:w-5 text-purple-400" /></div>
                  <div>
                    <p className="text-xs md:text-sm font-medium text-foreground">Video Generation</p>
                    <p className="text-xs text-muted-foreground">Turn images into cinematic videos</p>
                  </div>
                </div>

                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-lg border border-border bg-card/60">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-pink-600/15 flex items-center justify-center flex-shrink-0"><ImageIcon className="h-4 w-4 md:h-5 md:w-5 text-pink-400" /></div>
                  <div>
                    <p className="text-xs md:text-sm font-medium text-foreground">Version Control</p>
                    <p className="text-xs text-muted-foreground">Track and manage iterations</p>
                  </div>
                </div>

                <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-lg border border-border bg-card/60">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-green-600/15 flex items-center justify-center flex-shrink-0"><Eye className="h-4 w-4 md:h-5 md:w-5 text-green-400" /></div>
                  <div>
                    <p className="text-xs md:text-sm font-medium text-foreground">Real‑time Preview</p>
              ...
              {/* Metrics */}
              <div className="flex items-center justify-center lg:justify-start gap-6 md:gap-10 pt-2">
                <div className="text-center">
                  <div className="text-xl md:text-2xl font-bold text-foreground">10k+</div>
                ...
                <div className="text-center">
                  <div className="text-xl md:text-2xl font-bold text-foreground">1k+</div>
            ...
            <div className="w-full mx-auto max-w-lg lg:ml-auto order-first lg:order-last">
              <div className="bg-card/70 backdrop-blur-xl border border-border rounded-2xl p-2 shadow-2xl">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  ...
                  />
                  <Button
                    className="bg-red-600 hover:bg-red-600/90 text-white px-4 md:px-5 py-2 md:py-2.5 rounded-xl text-sm font-medium inline-flex items-center justify-center gap-2 touch-manipulation"
                    onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
                  >
                    TRY NOW <ArrowRight className="h-3 w-3 md:h-4 md:w-4" />
                  </Button>
                </div>
              </div>
      ...
      <section id="how-it-works">
        <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 py-12 md:py-16 lg:py-20">
          ...
          <div className="flex flex-col sm:flex-row gap-2 md:gap-3 mt-8 justify-center">
            <Button
              size="lg"
              className="bg-primary text-primary-foreground hover:bg-primary/90 h-10 md:h-12 px-6 md:px-8 text-sm md:text-base"
              onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
            >
              Start now
            </Button>
            <a href="#features">
              <Button size="lg" variant="outline" className="border-border h-10 md:h-12 px-6 md:px-8 text-sm md:text-base">Explore features</Button>
            </a>
          </div>
        </div>
      </section>
      ...
      <section id="security" className="border-t border-border/60 bg-card/40">
        <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 py-12 md:py-16 lg:py-20">
          <div className="grid md:grid-cols-2 gap-6 md:gap-10 items-start">
            <div>
              <h2 className="text-xl md:text-2xl font-semibold text-foreground">Enterprise posture</h2>
            ...
            <ul className="space-y-2 md:space-y-3 text-sm text-foreground/90">
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-primary flex-shrink-0" /> OAuth via Google</li>
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-primary flex-shrink-0" /> Local‑first during MVP</li>
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-primary flex-shrink-0" /> API keys kept server‑side</li>
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-primary flex-shrink-0" /> Export‑ready assets</li>
            </ul>
          </div>
        </div>
      </section>
  ...
  return (
    <div className="rounded-xl border border-border bg-card/60 p-4 md:p-5">
      <div className="flex items-start gap-3 md:gap-4">
        <img src={icon} alt="" className="h-5 w-5 md:h-6 md:w-6 opacity-80 flex-shrink-0" />
        <div>
...
function Step({ index, title, description }: { index: number; title: string; description: string }) {
  return (
    <div className="rounded-xl border border-border bg-card/60 p-4 md:p-6">
      <div className="flex items-start gap-3 md:gap-4">
        <div className="h-7 w-7 md:h-8 md:w-8 rounded-full border border-border flex items-center justify-center text-sm text-foreground flex-shrink-0">{index}</div>
        <div>
          <h3 className="text-sm md:text-base font-medium text-foreground">{title}</h3>
          <p className="mt-1 text-xs md:text-sm text-muted-foreground">{description}</p>
        </div>
